...
export default class DataAccess {
    ...
    write = async data => {
        ...
        for (let i = 0; i < contents.length; i++) {
            this.messages[i] ? this.messages[i].edit(contents[i]) : this.channel.send(contents[i])
        }
        const messagesToDelete = this.messages.slice(contents.length)
        messagesToDelete.forEach(_ => { _.delete() });
    }
    ...
    #getIndex = message => {
        ...
        if (isNaN(index)) throw new Error(`Index could not be parsed for message '${message.id}' (content: '${message.content}')`)
        return index
    }
    ...
    #getFullPrefix = messageNumber => `${dataMarker}${this.prefix}${placeHolder}${messageNumber}${newLine}`
